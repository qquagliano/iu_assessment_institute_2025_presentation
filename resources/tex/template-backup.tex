...
% In beamer background-image does not work well when other images are
% used, so this is the workaround
...
$if(selnolig-langs)$
\ifLuaTeX
\usepackage[$for(selnolig-langs)$$it$$sep$,$endfor$]{selnolig} %
...
\renewcommand\thesection{\color{gvblue}\arabic{section}} % NOTE:
% Change section number color
...
% SECTION: Bleeding edge forms code
